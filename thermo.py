...
def calc_capital_read(cfg, indices, breadth, emotion, liquidity, margin, sector, flow, mb, ratio, futures_basis=None, etf_flow=None):
    ...
    # 文字必须带具体数值与阈值，拒绝空口结论。
    main_net = fl.get("main_net", 0) or 0
    xlarge_net = fl.get("xlarge_net", 0) or 0
    small_net = fl.get("small_net", 0) or 0
    adv_pct = mb.get("advance_pct", 50) or 50
    trend = (breadth or {}).get("trend", "—") or "—"
    # 这些字段可能为 None；字符串在元组创建时即格式化，需先给安全默认值。
    # 真正触发该证据时条件已保证非 None/非空，安全值不会被展示。
    avg_bp_val = avg_bp if avg_bp is not None else 0.0
    ef_sum_val = ef_sum if ef_sum is not None else 0.0
    ratio_val = ratio if ratio is not None else 1.0
    regimes = [
        ("共振拉升", [
            (idx_up, 2, "上证 %+0.2f%%（红盘）" % sh_pct),
            (idx_strong, 1, "上证 %+0.2f%%（>+0.5%%，偏强）" % sh_pct),
            (width_high, 2, "成交额宽度 %.1f%%（>60%%，多数个股走强）" % bw),
            (main_in, 2, "主力净流入 +%.1f亿（>+%.0f亿）" % (main_net, flow_in)),
            (xl_in, 1.5, "超大单净流入 +%.1f亿（>+%.0f亿，机构买入）" % (xlarge_net, xl_in_th)),
            (emo_hot, 1, "情绪 %.0f分（≥58分 偏热但未狂热）" % es),
            (adv_high_b, 1, "上涨家数占比 %.1f%%（>%.0f%%）" % (adv_pct, adv_high_th)),
            (limit_many_b, 0.5, "涨停 %d家（≥%d家，涨停数偏多）" % (lu, limit_many_th)),
            (basis_bull, 1, "期指升水 %+.2f%%（>+%.2f%%，主力看多后市）" % (avg_bp_val, prem_th)),
            (etf_in, 1, "ETF 净流入 +%.1f亿（>+%.0f亿，机构借道布局）" % (ef_sum_val, etf_th)),
        ]),
        ("诱多派发", [
            (idx_up or idx_flat, 3, "上证 %+.2f%%（≥%.2f%% 红或平，指数不弱）" % (sh_pct, -0.3)),
            (width_low or width_mid, 1, "成交额宽度 %.1f%%（未>60%%，个股分化未普涨）" % bw),
            (main_out, 1.5, "主力净流出 %.1f亿（<%.0f亿）" % (main_net, flow_out)),
            (sm_in, 1.5, "小单净流入 +%.1f亿（>+%.0f亿，散户净买入/接盘）" % (small_net, sm_in_th)),
            (emo_hot, 1.5, "情绪 %.0f分（≥58分 偏热）" % es),
            (limit_many_b, 1, "涨停 %d家（≥%d家，高涨停数下炸板风险上升）" % (lu, limit_many_th)),
            (head_strong, 0.5, "宽度结构：%s（拉抬集中在头部权重）" % trend),
            (basis_bear, 1, "期指贴水 %+.2f%%（<%.2f%%，主力后市谨慎）" % (avg_bp_val, disc_th)),
        ]),
        ("出货撤退", [
            (idx_down, 2, "上证 %+.2f%%（绿盘）" % sh_pct),
            (main_out, 2, "主力净流出 %.1f亿（<%.0f亿）" % (main_net, flow_out)),
            (xl_out, 1.5, "超大单净流出 %.1f亿（<%.0f亿，机构卖出）" % (xlarge_net, xl_out_th)),
            (width_low, 1.5, "成交额宽度 %.1f%%（<%.0f%%）" % (bw, adv_low_th)),
            (emo_cool or emo_fear, 1.5, "情绪 %.0f分（≤42分 转冷/冰点）" % es),
            (limit_down_many_b, 1, "跌停 %d家（≥%d家）" % (ld, limit_down_many_th)),
            (liq_high, 0.5, "流动性风险 %.0f分（≥70分 高风险）" % lr),
            (basis_bear, 1.5, "期指贴水 %+.2f%%（<%.2f%%，避险情绪）" % (avg_bp_val, disc_th)),
            (etf_out, 1, "ETF 净流出 %.1f亿（<%.0f亿，资金撤离）" % (ef_sum_val, -etf_th)),
        ]),
        ("洗盘震仓", [
            (idx_down or idx_flat, 1, "上证 %+.2f%%（在±0.3%% 震荡/小跌）" % sh_pct),
            (width_mid, 1, "成交额宽度 %.1f%%（40~60%% 中性，未恶化）" % bw),
            (xl_in, 1, "超大单净流入 +%.1f亿（>+%.0f亿，机构仍在买）" % (xlarge_net, xl_in_th)),
            (emo_neutral, 1, "情绪 %.0f分（42~58分 中性）" % es),
            (vol_shrink, 1, "量能比 %.2fx（<%.2fx 缩量，无恐慌抛压）" % (ratio_val, vs)),
            ((main_out and not xl_out), 0.5, "大单净流出 %.1f亿，但超大单净流入 +%.1f亿（机构未撤）" % (main_net - xlarge_net, xlarge_net)),
            (basis_bull, 0.5, "期指升水 %+.2f%%（>+%.2f%%，非系统性风险）" % (avg_bp_val, prem_th)),
        ]),
        ("吸筹布局", [
            (idx_down or idx_flat, 1, "上证 %+.2f%%（震荡/小跌）" % sh_pct),
            (main_in, 1.5, "主力净流入 +%.1f亿（>+%.0f亿）" % (main_net, flow_in)),
            (xl_in, 1, "超大单净流入 +%.1f亿（>+%.0f亿，机构低位吸筹）" % (xlarge_net, xl_in_th)),
            (sm_out, 1.5, "小单净流出 %.1f亿（<%.0f亿，散户割肉）" % (small_net, sm_out_th)),
            (emo_cool or emo_fear, 1.5, "情绪 %.0f分（≤42分 偏冷/冰点）" % es),
            (width_low, 1, "成交额宽度 %.1f%%（<%.0f%%，偏低未启动）" % (bw, adv_low_th)),
            (etf_in, 1, "ETF 净流入 +%.1f亿（>+%.0f亿，机构借道布局）" % (ef_sum_val, etf_th)),
        ]),
        ("弱势阴跌", [
            (idx_down, 2, "上证 %+.2f%%（绿盘）" % sh_pct),
            (main_out, 2, "主力净流出 %.1f亿（<%.0f亿）" % (main_net, flow_out)),
            (width_low, 2, "成交额宽度 %.1f%%（<%.0f%%）" % (bw, adv_low_th)),
            (emo_fear, 1.5, "情绪 %.0f分（≤30分 冰点）" % es),
            (limit_down_many_b, 1, "跌停 %d家（≥%d家）" % (ld, limit_down_many_th)),
            (liq_high, 1, "流动性风险 %.0f分（≥70分 高风险）" % lr),
            (adv_low_b, 1, "上涨家数占比 %.1f%%（<%.0f%%）" % (adv_pct, adv_low_th)),
            (basis_bear, 1, "期指贴水 %+.2f%%（<%.2f%%，资金避险）" % (avg_bp_val, disc_th)),
            (etf_out, 1, "ETF 净流出 %.1f亿（<%.0f亿）" % (ef_sum_val, -etf_th)),
        ]),
    ]
